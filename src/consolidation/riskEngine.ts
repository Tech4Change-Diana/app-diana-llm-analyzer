/**
 * DIANA — Risk Engine (consolidação determinística — P2).
 *
 * Portado de `app-diana-monitoring/src/risk-engine/index.ts`. Consolida sinais,
 * fatores contextuais e severidade em um `RiskAssessment` (frequência,
 * sequência, escalada, combinação, severidade, confiança -> score/prioridade).
 * Ver `docs/regras-de-negocio.md` §§5–6.
 *
 * O mapa sinal->categoria e a ordem de categorias vêm de `@diana/contracts`
 * (`taxonomy.ts`), fonte única compartilhada com a taxonomia da LLM.
 *
 * ⚠️ MOCK / PROTOTYPE — heurística de demonstração, não fórmula validada.
 */
import type {
  ContextualFactor,
...
import { CATEGORY_ORDER, categoryForSignalType } from "@diana/contracts";
import {
...
export function evaluateRisk(
  ...
  for (const signal of signals) {
    const category = categoryForSignalType(signal.type);
  ...
  const rationale =
    signals.length === 0
      ...
      : `Foram identificados ${signals.length} sinais relevantes (${distinctTypes.join(", ")}), ` +
        ...
        "Valores técnicos consolidados de forma determinística.";
