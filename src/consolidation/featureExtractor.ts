/**
 * DIANA — Extração de características (heurística regex, PT-BR).
 *
 * Portado de `app-diana-monitoring/src/pipeline/featureExtractor.ts`. Usado pelo
 * `MockRiskAnalyzer` (fallback/baseline). As heurísticas são uma abstração de
 * MVP e NÃO representam um modelo validado. Ver `docs/regras-de-negocio.md` §2.2.
 */
...
export const SIGNAL_PATTERNS: Record<string, RegExp[]> = {
  ...
  personal_information_shared: [/escola\s+municipal/, /moro\s+na\s+rua/, /moro\s+no\s+bairro/],
